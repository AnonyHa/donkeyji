...
void evsignal_init(struct event_base* base)
{
	...
	for (i=0; i<NSIG; i++) TAILQ_INIT(&base->sig.evsigevents[i]);
...
int evsignal_add(struct event* ev)
{
	...
	if (TAILQ_EMPTY(&sig->evsigevents[evsignal])) {
		...
		if (_evsignal_set_handler(base, evsignal, evsignal_handler) == -1) return -1;//设置信号处理函数

		evsignal_base = base;

		//sig->ev_signal仅需event_add一次
		if (!sig->ev_signal_added) {
			if (event_add(&sig->ev_signal, NULL) != 0) return -1;
